...
const GetCardDetailsUser = async (req, res) => {
    try {
        ...
        const cardRequest = await cardAllotments.findOne({ _id: new ObjectId(id) });
        if (!cardRequest) {
            return res.status(404).send("Card request not found");
        }

        const rfidCard = await rfidStore.findOne({ _id: new ObjectId(cardRequest.rfidId) });
        if (!rfidCard) {
...
module.exports = { GetCardDetailsUser };
